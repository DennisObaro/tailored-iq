...
export interface CoverflowCarouselProps {
  ...
  gap?: number;
  loop?: boolean;
  /** Advance one card at a time on a timer. Pauses on hover and while
   *  dragging, and is disabled outright under prefers-reduced-motion. */
  autoPlay?: boolean;
...
showPagination = false,
  ...
  /** Hover pauses auto-play; a ref (not state) since it doesn't need to
      trigger a render, only to be read by the next timer tick. */
  ...
  // One card forward on a timer. Skips a tick rather than resetting it while
  // a drag or hover is in progress (or the tab is backgrounded), so coming
  // back doesn't cause a multi-card catch-up jump.
  React.useEffect(() => {
    if (!autoPlay || reducedMotion || count < 2) return;
    const id = window.setInterval(() => {
      if (dragRef.current || hoveredRef.current || document.hidden) return;
      nudge(1);
    }, autoPlayInterval);
    return () => window.clearInterval(id);
  }, [autoPlay, autoPlayInterval, count, reducedMotion, nudge]);
